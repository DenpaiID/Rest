...
const os = require('os');
const fs = require('fs');
const denpai = require('./function/index') 
...
app.get('/api/ragbot', async (req, res) => {
  try {
    ...
    const response = await denpai.ragBot(message);
    res.status(200).json({
      status: 200,
      creator: "siputzx",
      data: { response }
...
app.get('/api/degreeguru', async (req, res) => {
  try {
    ...
    const response = await denpai.degreeGuru(message);
    res.status(200).json({
      status: 200,
      creator: "siputzx",
      data: { response }
...
app.get('/api/smartcontract', async (req, res) => {
  try {
    ...
    const response = await denpai.smartContract(message);
    res.status(200).json({
      status: 200,
      creator: "siputzx",
      data: { response }
...
app.get('/api/blackboxAIChat', async (req, res) => {
  try {
    ...
    const response = await denpai.blackboxAIChat(message);
    res.status(200).json({
      status: 200,
      creator: "siputzx",
      data: { response }
